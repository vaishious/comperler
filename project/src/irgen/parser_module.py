...
class Parser(object):
    ...
    def p_error(self, p):
        if not self.error_seen:
            if p is not None:
                print("Parser encountered a syntax error in input at Line %d."%(p.lineno))
    ...
    def p_list_elements_error(self, p):
        ''' list-elements : error COMMA arith-bool-string-expression
        '''
        self.error_list.append((p.lineno(2), "Invalid list elements at line %d"%(p.lineno(2))))       
    ...
    def p_hash_elements_error(self, p):
        ''' hash-elements : error COMMA arith-bool-string-expression HASHARROW arith-bool-string-expression
                          | error HASHARROW arith-bool-string-expression
        '''
    ...
    def p_arith_bool_string_expression_error(self, p):
        ''' arith-bool-string-expression : LPAREN error RPAREN
        '''
        self.error_list.append((p.lineno(3), "Invalid Expression starting at line %d and ending at line %d"%(p.lineno(1),p.lineno(3))))       
    ...
    def p_if_elsif_error(self, p):
        ''' if-elsif : IF LPAREN error RPAREN MARK-backpatch codeblock
                     | IF LPAREN error RPAREN MARK-backpatch codeblock MARK-backpatch-nextlist MARK-backpatch elsif '''
    ...
    def p_unless_error(self, p):
        ''' unless-elsif : UNLESS LPAREN error RPAREN MARK-backpatch codeblock MARK-backpatch-nextlist MARK-backpatch elsif
                         | UNLESS LPAREN error RPAREN MARK-backpatch codeblock '''
    ...
    def p_while_loop_error(self, p):
        ''' while-loop : WHILE MARK-backpatch LPAREN error RPAREN MARK-backpatch codeblock '''
        self.error_list.append((p.lineno(4), "Line %d: Invalid conditional passed to WHILE loop"%(p.lineno(4))))
    ...
    def p_while_loop_continue_error(self, p):
        ''' while-loop : WHILE MARK-backpatch LPAREN error RPAREN MARK-backpatch codeblock MARK-backpatch continue '''
    ...
    def p_until_loop_error(self, p):
        ''' until-loop : UNTIL MARK-backpatch LPAREN error RPAREN MARK-backpatch codeblock '''
        self.error_list.append((p.lineno(4), "Line %d: Invalid conditional passed to UNTIL loop"%(p.lineno(4))))
    ...
    def p_until_loop_continue_error(self, p):
        ''' until-loop : UNTIL MARK-backpatch LPAREN error RPAREN MARK-backpatch codeblock MARK-backpatch continue '''
        self.error_list.append((p.lineno(4), "Line %d: Invalid conditional passed to UNTIL loop"%(p.lineno(4))))
    ...
    def p_do_while_loop_error(self, p):
        # Next, redo and last give a compilation error when used with do-while
        ''' do-while-loop : DO MARK-backpatch codeblock MARK-backpatch WHILE LPAREN error RPAREN SEMICOLON '''
        self.error_list.append((p.lineno(7), "Line %d: Invalid conditional passed to DO WHILE loop"%(p.lineno(7))))
    ...
    def p_foreach_loop_error1(self, p):
        ''' foreach-loop : FOREACH error LPAREN var-lhs RPAREN MARK-backpatch codeblock '''
        self.error_list.append((p.lineno(2), "Line %d: Invalid variable name in to FOREACH loop"%(p.lineno(2))))

    def p_foreach_loop_error2(self, p):
        ''' foreach-loop : FOREACH var-lhs LPAREN error RPAREN MARK-backpatch codeblock '''
        self.error_list.append((p.lineno(4), "Line %d: Invalid list given to the FOREACH loop"%(p.lineno(4))))
    ...
    def p_foreach_loop_continue_error1(self, p):
        ''' foreach-loop : FOREACH error LPAREN var-lhs RPAREN MARK-backpatch codeblock MARK-backpatch continue '''
        self.error_list.append((p.lineno(2), "Line %d: Invalid variable name in to FOREACH loop"%(p.lineno(2))))

    def p_foreach_loop_continue_error2(self, p):
        ''' foreach-loop : FOREACH var-lhs LPAREN error RPAREN MARK-backpatch codeblock MARK-backpatch continue '''
        self.error_list.append((p.lineno(4), "Line %d: Invalid list given to the FOREACH loop"%(p.lineno(4))))
    ...
    def p_ternary_operator_error1(self, p):
        ''' ternary-op : error TERNARY_CONDOP MARK-backpatch usable-expression MARK-ternary-assignment1 MARK-backpatch-nextlist COLON MARK-backpatch usable-expression 
        '''
        self.error_list.append((p.lineno(1), "Line %d: Invalid Conditional given to Ternary Operator (?:)"%(p.lineno(1))))

    def p_ternary_operator_error2(self, p):
        ''' ternary-op : boolean-expression TERNARY_CONDOP MARK-backpatch error MARK-ternary-assignment1 MARK-backpatch-nextlist COLON MARK-backpatch usable-expression 
        '''
        self.error_list.append((p.lineno(4), "Line %d: Expression (under then) given to Ternary Operator (?:) is incorrect"%(p.lineno(4))))
